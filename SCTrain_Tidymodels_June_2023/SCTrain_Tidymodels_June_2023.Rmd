...
In case of unsupervised models the model there are no such data and it tries to naturally find the best solution to the problem. Such models are for example k-nearest neighbors clustering, self-organizing maps, principal component analysis or auto-encoders.
...
Depending on a person You're talking to You would get different answer about how their modeling workflow, or life-cycle looks like.
...
9. Repeat with other models or hyper-parameter settings
...
This behaviour can be overridden by argument `breaks` which takes single number as input indicating the number of bins for stratification.
